...
--
-- Note: This version properly handles functions with signature changes
--
...
DROP FUNCTION IF EXISTS validate_routine_variable_trigger();
...
-- Fix create_routine_auto_logs function - match existing signature exactly
DROP FUNCTION IF EXISTS create_routine_auto_logs(DATE);
CREATE OR REPLACE FUNCTION create_routine_auto_logs(target_date DATE DEFAULT CURRENT_DATE)
RETURNS TABLE(routine_id UUID, routine_name TEXT, variable_name TEXT, auto_logged BOOLEAN, error_message TEXT) 
SET search_path = ''
AS $$
DECLARE
    routine_record RECORD;
...
BEGIN
    ...
    -- Loop through all active routines that should run today
    FOR routine_record IN
        ...
        WHERE r.is_active = true
          AND (r.weekdays IS NULL OR target_weekday = ANY(r.weekdays))
    LOOP
        -- Check if auto-logs already exist for this routine today
        SELECT EXISTS(
            SELECT 1 
            FROM routine_log_history rlh
            WHERE rlh.routine_id = routine_record.id
              AND rlh.log_date = target_date
              AND rlh.was_overridden = false
        ) INTO log_exists;

        -- If no auto-logs exist, create them
        IF NOT log_exists THEN
            -- Insert auto-logs for all variables in this routine
            ...
            LOOP
                -- Check if there's already a manual log for this variable today
                SELECT EXISTS(
                    SELECT 1 
                    FROM data_points dp
                    WHERE dp.user_id = routine_record.user_id
                      AND dp.variable_id = variable_record.variable_id
                      AND dp.date = target_date
                ) INTO manual_log_exists;

                -- Only create auto-log if no manual log exists
                IF NOT manual_log_exists THEN
                    BEGIN
                        INSERT INTO routine_log_history (
                            ...
                            routine_record.id,
                            NULL, -- We don't have routine_variable_id in this context
                ...
                ELSE
                    ...
                    variable_name := variable_record.variable_name;
                    auto_logged := false;
                    error_message := 'Manual log already exists';
                    RETURN NEXT;
                END IF;
            END LOOP;
        END IF;
...
DROP FUNCTION IF EXISTS trigger_routine_notifications();
...
DROP FUNCTION IF EXISTS get_oura_variable_id(TEXT);
...
DROP FUNCTION IF EXISTS trigger_handle_routine_override();
...
DROP FUNCTION IF EXISTS get_shared_variables(UUID);
...
DROP FUNCTION IF EXISTS get_unit_display_info(TEXT);
...
DROP FUNCTION IF EXISTS seed_variable_units();
