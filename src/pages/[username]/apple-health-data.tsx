...
import { ArrowBack, Apple } from '@mui/icons-material';
...
export default function AppleHealthDataPage() {
  const router = useRouter();
  const { username } = router.query;
  
  const [dataPoints, setDataPoints] = useState<AppleHealthDataPoint[]>([]);
  ...
  const getVariableLabel = (variableId: string) => {
    const mapping: Record<string, string> = {
      ...
      'ah_body_mass': 'Body Mass',
      'ah_body_fat_percentage': 'Body Fat %',
      'ah_blood_pressure_systolic': 'Blood Pressure (Systolic)',
      'ah_blood_pressure_diastolic': 'Blood Pressure (Diastolic)',
      'ah_vo2_max': 'VO2 Max'
    };
    return mapping[variableId] || variableId;
  };
  ...
  const getVariableUnit = (variableId: string) => {
    const units: Record<string, string> = {
      ...
      'ah_heart_rate': 'bpm',
      'ah_weight': 'kg',
      'ah_sleep_duration': 'hours',
      'ah_active_calories': 'cal',
      'ah_resting_heart_rate': 'bpm',
      'ah_body_mass': 'kg',
      'ah_body_fat_percentage': '%',
      'ah_blood_pressure_systolic': 'mmHg',
      'ah_blood_pressure_diastolic': 'mmHg',
      'ah_vo2_max': 'mL/kg/min'
    };
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        console.error('Error loading Apple Health data:', err);
        setError(err.message || 'Failed to load Apple Health data');
      } finally {
        setLoading(false);
      }
    };
    ...
    <Container maxWidth="xl" className="py-8">
      ...
      <Box className="mb-6">
        ...
        <Box display="flex" alignItems="center" gap={2} className="mb-4">
          <Apple className="text-blue-500" fontSize="large" />
        ...
        <Typography variant="h6" className="text-gray-400 mb-4">
          @{username} • Apple Health Integration
        </Typography>
      </Box>
      ...
      <Card className="bg-surface border border-border">
        <CardContent>
          <Typography variant="h6" className="text-gold mb-4">
            Recent Apple Health Data (Page {page} of {totalPages})
          </Typography>
          
          {dataPoints.length === 0 ? (
            <Alert severity="info">
              No Apple Health data found for this user.
            </Alert>
          ) : (
            <>
                ...
                <Table>
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell className="text-gold font-bold">Source Info</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {dataPoints.map((point) => (
                      <TableRow key={point.id} className="hover:bg-surface">
                        ...
                        <TableCell className="text-gray-400">
                          {point.raw?.from_ios_app ? (
                            <Chip
                              label={`iOS App v${point.raw?.app_version || 'unknown'}`}
                              size="small"
