...
interface LogEntry {
  ...
  variable: string;
  variable_id?: string; // UUID field for new logs table
  value: string;
  notes?: string;
...
export default function LogPage() {
  ...
  const getVariableNameFromLog = (log: LogEntry): string => {
  ...
  const getVariableIconFromLog = (log: LogEntry): string => {
    // If log has variable_id, look up the variable in the variables array
    if (log.variable_id && variables.length > 0) {
      const variable = variables.find((v) => v.id === log.variable_id);
      if (variable) {
      ...
      <Container maxWidth="md" sx={{ py: 4 }}>
        <Box sx={{ textAlign: "center" }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Fetching variables, experiments, and today's logs
          </Typography>
        </Box>
      </Container>
    );
